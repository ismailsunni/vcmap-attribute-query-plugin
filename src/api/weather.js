...
export default class Weather {
  constructor(options) {
    ...
    this.snowUpdateFunction = (particle) => {
      ...
      if (distance > this.radius) {
        ...
        particle.endColor.alpha =
          this.snowSystem.endColor.alpha / (distance / this.radius + 0.1);
      }
    };
    this.rainUpdateFunction = (particle) => {
      ...
      if (distance > this.radius) {
        ...
        particle.endColor.alpha = 0.7 / (distance / this.radius + 0.1);
      }
    };
  }
   ...
   * @returns {vcs.vcm.plugins.weather.Weather} instance of Weather
   */
  ...
  updateLocation() {
    ...
    this.snowSystem.modelMatrix = Cesium.Matrix4.setTranslation(
      modelMatrix,
      this.position,
      modelMatrix,
    );
    this.rainSystem.modelMatrix = Cesium.Matrix4.setTranslation(
      modelMatrix,
      this.position,
      modelMatrix,
    );
